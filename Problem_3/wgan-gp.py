...
def compute_gp(true_picture, fake_pictures, critic):
    ...
    merged_pictures = autograd.Variable(merged_pictures, requires_grad=True)

    out_merged = critic(merged_pictures)

    gradient_outputs = torch.ones(out_merged.size())

    if cuda:
        gradient_outputs = gradient_outputs.cuda()

    gradients = torch.autograd.grad(outputs=out_merged.sum(), inputs=merged_pictures,
                                    create_graph=True)[0]

    gradients = gradients.contiguous().view(gradients.size(0), -1)

    gp = torch.pow((gradients.norm(2, dim=1) -1),2).mean()
...
def train_wgan():
    ...
    parser.add_argument('--start_iteration', type=int, default=97,
    ...
    parser.add_argument('--lr_critic', type=float, default=0.00005,
                        help='Generator Learning Rate')
    parser.add_argument('--n_critic', type=float, default=5,   #5 in the paper, but the TA told us that we might need to tweak this one.
    ...
    optimizer_critic = Adam(params_critic,lr=args.lr_critic, betas=(0.5,0.9))

    params_generator = generator.parameters()
    optimizer_generator = Adam(params_generator, lr=args.lr_generator, betas=(0.5,0.9))
    ...
    for epoch in range(args.start_iteration, args.max_iteration):
        ...
        for i, (true_pictures, y) in enumerate(train):
            ...
            out_fake = critic(fake_pictures.detach())
