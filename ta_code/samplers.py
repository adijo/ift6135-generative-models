...
def distribution1(x, batch_size=512):
    ...
    return np.array([(x, random.uniform(0, 1)) for _ in range(batch_size)])


def distribution2(batch_size=512):
    # High dimension uniform distribution
    return np.random.uniform(0, 1, (batch_size, 2))


def distribution3(batch_size=512):
    # 1D gaussian distribution
    return np.random.normal(0, 1, (batch_size, 1))
...
def distribution4(batch_size=1):
    ...
    return f(np.random.normal(0, 1, (batch_size, 1)))
