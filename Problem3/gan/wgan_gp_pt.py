...
class Critic(nn.Module):
    def __init__(self, image_shape=(3, 32, 32), dim_factor=64):
        ...
        self.critic = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Conv2d(64,128,kernel_size=3, stride=1, padding=1), #This layer is to catch small features
            nn.LeakyReLU(0.2),
            nn.ZeroPad2d(2),
...
class ClassifierGAN22(nn.Module):
    def __init__(self):
        super(ClassifierGAN22, self).__init__()
...
def train_wgan():
    ...
    critic = ClassifierGAN()
    if args.pretrained_critic is not None:
        print("Loading pre-trained weights: " + args.pretrained_critic)
        parameters = torch.load(args.pretrained_critic).state_dict()
        critic_dict = critic.state_dict()
        for key, value in critic_dict.items():
            #print (key, value)
            if key in parameters:
                print(key)
                critic_dict[key]=parameters[key]
